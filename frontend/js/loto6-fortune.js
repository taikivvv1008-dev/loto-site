...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (
    ...
    console.log("❌ loto6-fortune.js: 必須要素が見つかりません");
    return;
  }
  ...
  const MODEL = "fortune";
  const USER_ID = "user001"; // ← Phase5でログイン連携に置換
  ...
  issueButton.addEventListener("click", async () => {
    ...
    try {
      ...
      // ✅ 予想番号は tickets をそのまま展開（包含ルールはエンジン/API側のsliceで保証）
  ...
  function renderFortune(json) {
    ...
    // 重複しやすいので main と line1/2 が両方ある時は main を落とす
  ...
  async function fetchLatestDraw(lotoType) {
    ...
    return await res.json(); // { round, draw_date, weekday, ... }
  }
  ...
  async function fetchFortuneOnce({ birthDateStr, count, round, drawDate }) {
    ...
    // ✅ ここは「単発JSON」前提。ticketsが無い形は異常として落とす（事故防止）
